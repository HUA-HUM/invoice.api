...
export interface DeleteInvoiceRepositoryOptions {
  baseUrl?: string;
...
export class XubioInvoiceDeleteRequestError extends Error {
  constructor(transaccionId: number, detail?: string) {
    super(
      `Xubio request failed while deleting invoice ${transaccionId}${detail === undefined ? '' : `: ${detail}`}`,
    );
    this.name = XubioInvoiceDeleteRequestError.name;
  }
}

export class DeleteInvoiceRepository implements IDeleteXubioInvoiceRepository {
  ...
  constructor(options: DeleteInvoiceRepositoryOptions = {}) {
  ...
  async delete(
    ...
    try {
      ...
      return {
        transaccionId: command.transaccionId,
        deleted: true,
...
function buildRequestError(
  ...
  if (axios.isAxiosError(error)) {
    ...
    return new XubioInvoiceDeleteRequestError(transaccionId, detail);
  }

  if (error instanceof Error) {
    return new XubioInvoiceDeleteRequestError(transaccionId, error.message);
  }

  return new XubioInvoiceDeleteRequestError(transaccionId, 'unknown error');
}
