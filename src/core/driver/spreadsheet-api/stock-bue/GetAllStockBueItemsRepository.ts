...
export class GetAllStockBueItemsRepository implements IGetAllStockBueItemsRepository {
  ...
  async getAll(
    ...
    let currentPage = 1;
    let firstPage: GetOneStockBueItemsResponse | undefined;
...
function validateStableMetadata(
  firstPage: GetOneStockBueItemsResponse,
  ...
  if (currentPage.totalRows !== firstPage.totalRows) {
    throw new StockBueSpreadsheetApiPaginationError(
      'totalRows changed between pages',
    );
  }
  if (currentPage.totalPages !== firstPage.totalPages) {
    throw new StockBueSpreadsheetApiPaginationError(
      'totalPages changed between pages',
    );
  }
}
